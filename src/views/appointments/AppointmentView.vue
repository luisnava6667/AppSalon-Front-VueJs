  ...
  <div class="space-y-8" v-if="!appointments.noServicesSelected">
    ...
    <div class="lg:flex gap-5 items-start">
      <div class="w-full lg:w-96 bg-white flex justify-center rounded-lg">
        <VueTailwindDatepicker :disable-date="disableDate" i18n="es-es" as-single no-input :formatter="formatter"
      ...
      <div class="flex-1 grid grid-cols-1 xl:grid-cols-2 gap-5 mt-10 lg:mt-0" v-if="appointments.isDateSelected">
        <button v-for="hour in appointments.hours"
          ...
          :class="appointments.time === hour ? 'bg-blue-500 text-white' : 'bg-white'" @click="appointments.time = hour"
          :disabled="appointments.disableTime(hour) ? true : false">{{ hour }}</button>
      </div>
    </div>
    <div class="flex justify-end" v-if="appointments.isValidReservation">
      <button class="w-full md:w-auto bg-blue-500 p-3 rounded-lg uppercase font-black text-white"
        @click="appointments.saveAppointment">Confirmar Reservación</button>
    </div>
  </div>
</template>